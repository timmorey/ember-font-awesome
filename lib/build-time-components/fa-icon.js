...
module.exports = class FaIconComponent extends BuildTimeComponent {
  ...
  iconContent() {
    if (this.invocationAttrs.icon) {
      if (isDynamic(this.invocationAttrs.icon)) {
        this.options.addon.fontAwesomeUsage.usedIconsUnknown = true;
        return buildConcatIfPresent(this.invocationAttrs.icon, ['fa-', this.invocationAttrs.icon]);
      } else {
        if (!this.options.addon.fontAwesomeUsage.usedIconsUnknown) {
          this.options.addon.fontAwesomeUsage.usedIcons.add(this.invocationAttrs.icon.value);
        }
        return `fa-${this.invocationAttrs.icon.value}`;
      }
    }
  }
