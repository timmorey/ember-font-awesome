...
let dynamicInvocationsFound = false;
let usedIcons = [];
module.exports = class AstTransform {
  ...
  transform(ast) {
    ...
    this.syntax.traverse(ast, {
      MustacheStatement(node) {
        ...
        let purelyStatic = node.hash.pairs.every(pair => pair.value.type !== "PathExpression");
          if (!purelyStatic) {
            dynamicInvocationsFound = true;
            return;
          }
        let classNames = ['fa'];
        let icon = node.hash.pairs.find(pair => pair.key === 'icon');
        if (icon) {
          usedIcons.push(icon.value.value)
    ...
    console.log('icons used:', usedIcons);
    console.log('dyamic invocation used:', dynamicInvocationsFound);
