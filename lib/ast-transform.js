...
/**
 * Tentative API for build-time components that process their inner block
 *
 * new BuildTimeComponent(node, {
 *   tagName: 'whatever',
 *   bodyVisitor: {
 *     MustacheStatement(node) {
 *       // this here should be the BuildTimeComponent but the function should also
 *       // retain access to anything available on this scope via closure.
 *       // That way the block could be very flexible and access pretty much everything.
 *     }
 *   }
 * });
 */
...
class AstTransform {
  ...
  transform(ast) {
    traverse(ast, {
      BlockStatement: (node) => {
        let component;
        let componentAlias = node.program.blockParams[0];
        if (node.path.original === 'fa-list') {
          component = new FaListComponent(node, {
            contentVisitor: {
              MustacheStatement: (node) => {
                if (node.path.original === `${componentAlias}.fa-icon`) {
          ...
          component = new FaStackComponent(node, {
            contentVisitor: {
              MustacheStatement: (node) => {
        ...
        return component && component.toNode();
      },
      ...
      MustacheStatement: (node) => {
        if (node.path.original === 'fa-icon') {
          ...
          return (new FaListComponent(node)).toNode();
        } else if (node.path.original === 'fa-stack') {
          return (new FaStackComponent(node)).toNode();
        }
      }
    });
