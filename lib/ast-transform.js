...
class FaIconComponent extends BuildTimeComponent {
  ...
  pullContent() {
    let pair = this.node.hash.pairs.find((p) => p.key === 'pull');
    if (pair) {
      if (isDynamic(pair.value)) {
        if (pair.value.type === 'PathExpression') {
          return b.concat([b.text('fa-pull-'), b.mustache(pair.value)]);
        } else if (pair.value.type === 'SubExpression') {
          return b.concat([b.text('fa-pull-'), b.mustache(pair.value.path, pair.value.params)]);
        }
      } else {
        return `fa-pull-${pair.value.value}`;
      }
    }
  }

  rotateContent() {
    let pair = this.node.hash.pairs.find((p) => p.key === 'rotate');
    if (pair) {
      if (isDynamic(pair.value)) {
        if (pair.value.type === 'PathExpression') {
          return b.concat([b.text('fa-rotate-'), b.mustache(pair.value)]);
        } else if (pair.value.type === 'SubExpression') {
          return b.concat([b.text('fa-rotate-'), b.mustache(pair.value.path, pair.value.params)]);
        }
      } else {
        return `fa-rotate-${pair.value.value}`;
      }
    }
  }

  flipContent() {
    let pair = this.node.hash.pairs.find((p) => p.key === 'flip');
    if (pair) {
      if (isDynamic(pair.value)) {
        if (pair.value.type === 'PathExpression') {
          return b.concat([b.text('fa-flip-'), b.mustache(pair.value)]);
        } else if (pair.value.type === 'SubExpression') {
          return b.concat([b.text('fa-flip-'), b.mustache(pair.value.path, pair.value.params)]);
        }
      } else {
        return `fa-flip-${pair.value.value}`;
      }
    }
  }
  ...
  stackContent() {
    // TODO: I think this is the last property to implement, but
    // it's not properly tested in the `fa-icon-test.js` file.
    // if (isDynamic(iconValue)) {
    ...
    traverse(node.program, {
      MustacheStatement: (node) => {
        if (node.path.original === `${listAlias}.fa-icon`) {
          let faIcon = new FaIconComponent(node, { listItem: true });
          return faIcon.toNode();
        }
      }
    });
