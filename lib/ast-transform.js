...
function transformFaIcon(b, node, { tagName = 'i', listItem = false, stack = undefined } = {}) {
  let attrs = [];
...
function transformFaListBlock(b, traverse, node) {
  let listAlias = node.program.blockParams[0];

  traverse(node.program, {
    MustacheStatement(node) {
      if (node.path.original === `${listAlias}.fa-icon`) {
        return transformFaIcon(b, node, { listItem: true });
      }
    }
  });

  return buildFaList(b, node);
}

function transformFaStackBlock(b, traverse, node) {
  let listAlias = node.program.blockParams[0];

  traverse(node.program, {
    MustacheStatement(node) {
      if (node.path.original === `${listAlias}.stack-1x`) {
        return transformFaIcon(b, node, { stack: b.string('1') });
      } else if (node.path.original === `${listAlias}.stack-2x`) {
        return transformFaIcon(b, node, { stack: b.string('2') });
      }
    }
  });

  return buildFaStack(b, node);
}
    ...
    traverse(ast, {
      BlockStatement(node) {
        if (node.path.original === 'fa-list') {
          return transformFaListBlock(b, traverse, node);
        } else if (node.path.original === 'fa-stack') {
          return transformFaStackBlock(b, traverse, node);
        }
      },

      MustacheStatement(node) {
        if (node.path.original === 'fa-icon') {
          return transformFaIcon(b, node);
